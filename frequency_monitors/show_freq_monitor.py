...
if __name__ == '__main__':
	...
	parser.add_argument('--ename',
						help='Input h5 epsilon filename', default='')
	...
	if args.ename != '':
		...
		cmap = ListedColormap(["navy", "black"])
		eps_plot = plt.imshow(eps, **default_eps_parameters, cmap=cmap,
	...
	field_plot = plt.imshow(fft_field[:,::-1, find_nearest(freqs, central_freq)].T, cmap=my_cmap,
							extent=[metadata['xmin'],
									metadata['xmax'],
									metadata['ymin'],
									metadata['ymax']])
